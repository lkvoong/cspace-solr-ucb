----------------------------------------------------------------------------------------------------
-- botgardenMetadataV1dead.sql
-- all objects, not deleted and is dead (deadflag = 'true')
-- exclude deleted collectionobjects
-- get deadflag = 'true' for is dead, i.e. dead accessions only
...
-- same query as alive, except:
--   cb.deadflag = 'true'
--   dead accessions should not have currentlocation (gardenLocation_s) not reasonformove (reasonForMove_s)
-- ~35513 rows
----------------------------------------------------------------------------------------------------
...
with objects as (
  select
    ...
    'yes' as deadFlag_s,
    to_char(cb.deaddate, 'YYYY-MM-DD') as deadDate_s,
...
-- objectid = 'ff218a66-b0f3-4fd7-a67a-7d7e7d90f791' has 6 determs
